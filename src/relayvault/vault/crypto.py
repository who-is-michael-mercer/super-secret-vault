...
def build_aad(
    vault_id: str, record_type: int | str, object_id: str | None = None
) -> bytes:
...
def parse_record_header(
    blob: bytes, record_size: int | None = None
) -> tuple[int, bytes]:
...
def decrypt_bytes(
    blob: bytes, key: bytes, aad: bytes, expected_record_type: int
) -> bytes:
    _validate_key(key)
...
def encrypt_stream(
    source: BinaryIO, destination: BinaryIO, key: bytes, aad: bytes, record_type: int
) -> None:
...
def decrypt_stream(
    ...
    try:
        ...
        source.seek(start)
        record_type, nonce = parse_record_header(
            _read_exact(source, HEADER_SIZE), end - start
        )
        if record_type != expected_record_type:
            raise VaultFormatError("Unexpected record type.")
...
def verify_stream(
    source: BinaryIO, key: bytes, aad: bytes, expected_record_type: int
) -> int:
    """Authenticate a bounded seekable record, discarding plaintext in memory."""
    _validate_key(key)
    _validate_record_type(expected_record_type)
    start = source.tell()
    end = source.seek(0, os.SEEK_END)
    source.seek(start)
    kind, nonce = parse_record_header(_read_exact(source, HEADER_SIZE), end - start)
    if kind != expected_record_type:
        raise VaultFormatError("Unexpected record type.")
    source.seek(end - TAG_SIZE)
    tag = _read_exact(source, TAG_SIZE)
    source.seek(start + HEADER_SIZE)
    ...
    while remaining:
        ...
        decryptor.update(_read_exact(source, amount))
        remaining -= amount
    try:
        decryptor.finalize()
    except InvalidTag as error:
